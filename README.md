...
Or alternatively, using [uv](https://github.com/astral-sh/uv):
...
If you prefer to use a package, see: [Packaging](package/readme.md).
...
```usage
usage: nerd-dictation [-h] {begin,end,cancel,suspend,resume,download} ...

positional arguments:
  {begin,end,cancel,suspend,resume,download}
    begin               Begin dictation.
    end                 End dictation.
    cancel              Cancel dictation.
    suspend             Suspend the dictation process.
    resume              Resume the dictation process.
    download            Download the VOSK model.
...
```usage
usage: nerd-dictation begin [-h] [--cookie FILE_PATH] [--config FILE] [--vosk-model-dir DIR] [--vosk-grammar-file DIR]
                            [--pulse-device-name IDENTIFIER] [--sample-rate HZ] [--defer-output] [--continuous] [--timeout SECONDS]
                            [--idle-time SECONDS] [--delay-exit SECONDS] [--suspend-on-start] [--punctuate-from-previous-timeout SECONDS]
                            [--full-sentence] [--numbers-as-digits] [--numbers-use-separator] [--numbers-min-value NUMBERS_MIN_VALUE]
                            [--numbers-no-suffix] [--input INPUT_METHOD] [--output OUTPUT_METHOD] [--simulate-input-tool SIMULATE_INPUT_TOOL]
                            [--verbose VERBOSE] [- ...]
...
```usage
usage: nerd-dictation end [-h] [--cookie FILE_PATH]
...
```usage
usage: nerd-dictation cancel [-h] [--cookie FILE_PATH]
...
```usage
usage: nerd-dictation suspend [-h] [--cookie FILE_PATH]
...
```usage
usage: nerd-dictation resume [-h] [--cookie FILE_PATH]
...
```usage
usage: nerd-dictation download [-h] [--model {small,large,lgraph,gigaspeech}] [--force] [-y] [--cookie FILE_PATH]
...
options:
  -h, --help            show this help message and exit
  --model {small,large,lgraph,gigaspeech}
                        The name of the model to download. Defaults to 'small'.
  ...
  --force               Force download and unzip the model even if one is already present.
  ...
  -y                    Confirm overwriting an existing model.
...
- [Elograf](https://github.com/papoteur-mga/elograf) - nerd-dictation GUI front-end that runs as a tray icon.
- [Numen](https://numen.johngebbie.com) - voice input for desktop computing that also uses VOSK.
